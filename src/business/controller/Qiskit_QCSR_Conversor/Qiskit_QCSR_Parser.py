...
class Python3Visitor(ast.NodeVisitor):
    ...
    def insertSimpleGate(self, gate, nodeArgs):
        QCSRgate = ''
        argument = ''
                          
        #Dependiendo del tipo de puerta, la traduce a QCSR y obtiene a partir del argumento donde se insertaria
            ...
            if isinstance(target, ast.Name):
                try:
                    ...
                    #Store the variable for the register of the circuit
                        ...
                        self.variables[target.id] = 'QuantumRegister' #stores value of qubits in dictionary
    ...
    def visit_Call(self, node):
        if isinstance(node.func, ast.Name):
            ...
            if gate in all_simple_gates:
                ...
                #Check if the argument its a QuantumRegister
                if isinstance(qubitArgument, ast.Name) and qubitArgument.id in self.variables and self.variables[qubitArgument.id] == 'QuantumRegister':
                    for qubit_index in range(0, len(self.content)): 
                        self.content[qubit_index].append(QCSRgate)
                #Or is a variable or a number
                else:
               ...
               #self.insertSimpleGate(gate, node.args)
        
            #Or if a complex gate was found
            if gate in all_complex_gates:
