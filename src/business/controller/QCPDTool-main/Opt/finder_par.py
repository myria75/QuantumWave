...
def get_cgates(col, ready):
    ...
    for i, gate in enumerate(col):
        if type(gate) == dict and 'CONTROL' in gate:
            ...
            if not paired:
                if col[gate['CONTROL']] == 'X' and ready[gate['CONTROL']]:
...
class PatternFinder:
        ...
        for i, qubit in enumerate(self.circuit):
            ...
            for j, gate in enumerate(qubit):
                ...
                if gate == '_':
                    ...
                    column = []
                    for k, row in enumerate(self.circuit):
                        try:
                            column.append(row[j])                        
                        except IndexError:
                            column.append('_')

                    if is_target(i, column):
                        break
    ...
    def find_superposition(self):
        ...
        # TODO: Up to here, the percetange is 50.00%
    ...
    def find_entanglement(self):
        ...
        for c, col in enumerate(trans):
            ...
            ready = [h_compare(ready[i], h_gates[i]) for i in range(len(col))] # superposed qubits
            c_gates, entities = get_cgates(col, ready)

            if c_gates != [-1 for _ in range(len(self.circuit))]: # Column has at least one ctrlled entity
                for i in range(len(self.circuit)):
                    if ready[i] != -1 and c_gates[i] != -1:
                        if f'{c}:{c_gates[i]}' in found.keys(): # Already found entanglement expansion
                            found[f'{c}:{c_gates[i]}'].append((i, (ready[i], entities[c_gates[i]][i])))
                        else:
        ...
        for f in found.items():
            qubit_no, col_no, fragment = craft_ent(f, len(self.circuit))
...
def main(circuit, verbose=False):
...
if __name__ == '__main__':
    circuit_48 = '[["H","_","_","_","_","_",{"CONTROL":1},{"CONTROL":2},"_",{"CONTROL":2},"_",{"CONTROL":1},{"CONTROL":2},"_",{"CONTROL":2},"_","X",{"CONTROL":1},{"CONTROL":2},"_",{"CONTROL":2},"_",{"CONTROL":1},{"CONTROL":2},"_",{"CONTROL":2},"_",{"CONTROL":3}],["H",{"CONTROL":2},"_",{"CONTROL":2},"_","X",{"CONTROL":2},"_","_","_","_",{"CONTROL":2},"_","_","_","_","_",{"CONTROL":2},"_","_","_","_",{"CONTROL":2},"_","_","_","_","_","H","MEASURE"],["_","X","RY","X","RY","_","X","X","RY","X","RY","X","X","RY","X","RY","_","X","X","RY","X","RY","X","X","RY","X","RY"],["_","_","_","_","_","_","_","_","_","_","_","_","_","_","_","_","_","_","_","_","_","_","_","_","_","_","_","X","_","MEASURE"]]'
    finder = PatternFinder(circuit_48)
    print(finder.find_entanglement())
